module Review::Filterable
  ...
  included do
    ...
    scope :by_date_range, ->(start_date, end_date) { where(review_date: start_date..end_date) if start_date && end_date }
  end
  ...
  class_methods do
    def filtered(params)
      ...
      scope = scope.by_date_range(params[:start_date], [:end_date])
      scope = scope.search_by_description(params[:q]) if params[:q].present?
      scope
    end
  end
end